...
const passwordsMatch = computed(() => !passwordConfirm.value || password.value === passwordConfirm.value)
const canSubmit = computed(
  () => email.value.trim().length > 3 && password.value.length >= 6 && password.value === passwordConfirm.value,
)
...
<template>
  <div class="auth-screen">
    <div class="auth-card glass-card" v-motion :initial="{ opacity: 0, y: 24 }" :enter="{ opacity: 1, y: 0, transition: { duration: 420 } }">
      ...
      <p class="auth-card__subtitle">Создайте аккаунт — первый сайт соберётся за минуту</p>
      ...
      <form class="auth-card__form" @submit.prevent="submit">
        ...
        <BaseInput v-model="password" type="password" label="Пароль" placeholder="Минимум 6 символов" autocomplete="new-password" />
        <BaseInput
        ...
        />
        <BaseButton type="submit" variant="primary" size="lg" block :loading="loading" :disabled="!canSubmit">
          Создать аккаунт
        </BaseButton>
      </form>
      ...
      <p class="auth-card__footer">
        Уже есть аккаунт?
        <NuxtLink to="/login" class="auth-card__link">Войти</NuxtLink>
      </p>
    </div>
  </div>
</template>
