...
def _extract_json(text: str) -> object | None:
    """YandexGPT, в отличие от `response_format=json_object` у OpenAI, не
    гарантирует чистый JSON без обрамления — модель иногда оборачивает ответ в
    ```json ... ``` или добавляет пояснение вокруг. Достаём JSON максимально
    терпимо вместо жёсткого json.loads(text)."""
    if not text:
        return None
...
async def _call_yandex_completion(
    ...
    try:
        ...
        # Сбой сети/лимитов/невалидный ответ — откатываемся на mock, чтобы один
        # упавший вызов не ронял всю генерацию сайта. Логируем, иначе причина
        # отката (истёкший ключ, 402 недостаточно средств и т.п.) не видна.
        body = exc.response.text[:500] if isinstance(exc, httpx.HTTPStatusError) else ""
        logger.warning("YandexGPT completion call failed: %s %s", exc, body)
        return None
...
class YandexCopywriter:
    ...
    async def _call_real_api(self, brief: BriefIn, block_types: list[str]) -> dict | None:  # pragma: no cover — требует ключ
        ...
        system_prompt = (
            ...
            "Пиши предметно: что предлагается, кому и как это работает. Заголовок hero — "
            "до 9 слов, подзаголовок — до 30 слов; у каждой секции своя задача без повторов. "
            "Не используй клише 'инновационные решения', 'индивидуальный подход', 'лучшее качество'. "
            "Не выдумывай клиентов, отзывы, награды, числа и цены: если фактов нет, оставляй "
            "списки отзывов и статистики пустыми, цены — 'По запросу'. Не используй emoji в иконках. "
            "Кнопки называй конкретным действием, согласованным с целью сайта. "
            f"{type_guidance}\n\n"
...
class YandexLayoutEngine:
    ...
    async def _call_real_api(self, brief: BriefIn) -> dict | None:  # pragma: no cover — требует ключ
        ...
        system_prompt = (
            ...
            "вариантов. Строй последовательность: предложение, детали продукта или работы, "
            "ответы на вопросы, целевое действие. Чередуй крупную композицию с компактной, "
            "не ставь три карточочные сетки подряд. Не добавляй отзывы и статистику без "
            "фактов в брифе. Предпочитай выразительную типографику и фотографии декоративным "
            "градиентам. Все блоки должны поддерживать одно визуальное направление. "
...
class YandexArtImageGenerator:
    ...
    async def generate_image(self, prompt: str) -> str:
        ...
        try:
            ...
            # Реальный вызов недоступен/упал — не роняем всю генерацию сайта
            # из-за одной картинки, откатываемся на детерминированный mock.
            return self._mock_image(prompt)
    ...
    async def _call_real_api(self, prompt: str) -> str:  # pragma: no cover — требует ключ
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(
                ...
                json={
                    "modelUri": f"art://{self.settings.yandex_folder_id}/{self.settings.yandex_art_model}",
                    "messages": [{"text": prompt, "weight": 1}],
                    "generationOptions": {
                        "mimeType": "image/jpeg",
                        "aspectRatio": {"widthRatio": 3, "heightRatio": 2},
                    },
                },
            )
            response.raise_for_status()
