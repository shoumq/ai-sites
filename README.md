...
### Дизайн-студия и предпочтения

Мастер создания собирает сферу бизнеса, аудиторию, тон текстов, визуальное
направление и нужные блоки в `BriefIn.preferences`. Генератор использует их
в промптах и резервной сборке, сохраняя существующий SiteSchema и ручные правки.
Подробнее о реализации и развитии: [Универсальный генератор](docs/generator-direction.md).

Проверка генератора (из `backend/` с установленными `requirements.txt`):
`python -m unittest discover -s tests -v`.

Проверка админки: `npm run typecheck` и `npm run build` из `admin-panel/`.
Для текущего Nuxt используйте Node 22.19+ или 24.11+.
